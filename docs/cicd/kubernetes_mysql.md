...
3. **稳定的网络标识**：
	...
	- 每个 Pod 都有一个稳定的主机名，格式为：<statefulset-name>-<ordinal-index>。

	- 例如：一个名为 web 的 StatefulSet 有三个副本，Pod 名称分别为 web-0、web-1、web-2。

	- 每个 Pod 拥有一个稳定的 DNS 名称：<pod-name>.<service-name>.<namespace>.svc.cluster.local。
...
| 网络标识 | 随机名称，不稳定 | 固定名称，有序（web-0, | web-1, | ...）|
| 存储 | 共享存储，Pod间无区别 | 独立存储，每个Pod专用 |
| 部署策略 | 滚动更新，可并行 | 顺序部署/删除（0→1→2）|
| 服务发现 | 通过Service负载均衡 | 通过Headless | Service固定访问特定实例 |
...
kubectl apply -f mysql-statefulset.yaml
```
![alt text](/assets/img/k8s_mysql_deleted.png)
...
![alt text](/assets/img/k8s_mysql_research.png)
